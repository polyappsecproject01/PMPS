	...
	<?php
		  ...
		  if (!empty($_SESSION["access"]))
		  {
			  if ($_SESSION["access"] == "admin") {
					...
					$userAccName = cleanData(trim($_POST["userAccountName"]));
					$userAccPW = cleanData(trim($_POST["userAccountPW"]));
					$userAccPW2= cleanData(trim($_POST["userAccountPW2"]));

					$pageAccType = cleanData(trim($_POST["userAccountType"]));
					$userAccType ="";

					// the type name used here is hardcode at html side.
					if ($pageAccType==='EMTType'){
						$userAccType = 'readonly';
					}else if($pageAccType==='DOCType'){
						 
						$userAccType = 'readwrite';
					}
					

		
					
					if ( (!empty($userAccName)) && (!empty($userAccPW)) && (!empty($userAccPW2))  ){
						if( $userAccPW !=$userAccPW2 ){
							print (" The two time you enter the password is not consistent.");
						}else if($userAccType==="admin"){
							//shoundn't enter into this section
							print (" The admin role can not be added.");
								
						}else if( ($userAccType!="readwrite") && ($userAccType!="readonly")){
							//shoundn't enter into this section
							print (" Please enter valid role type.");

						}else{
							$auth_dataArr = array('username' => $_SESSION["username"], 'ip_address' => $_SERVER['REMOTE_ADDR'], 'login_hash' => $_SESSION["loginHash"]);
							
							$requestArr = array('username' => $userAccName, 'accesslevel' => $userAccType, 'password' => $userAccPW);
							$arr = array('method' => 'addnewuser', 'auth_data' => $auth_dataArr,'request' => $requestArr );
							...
							$authNum = $responseArr["response"]["added"];
							$timeout = $responseArr["response"]["timeout"];
						
							if ($method == "addnewuser")  {
								if ($authNum === 1) {
									print ("<i>$userAccName account </i> was successfully added.");
								} else if ($authNum === 0){
									if($timeout ===1 ){
										print("Your session is timeout, you need relogin.");
									}else{
										print ("Error: Could not add <i>$userAccName</i> account.");	
									}
									
								}else{
									print("Error: added num.");
