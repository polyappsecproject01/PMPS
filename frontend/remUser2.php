	...
	<?php
		  ...
		  if (!empty($_SESSION["access"]))
		  {
			  if ($_SESSION["access"] == "admin") {
					...
					$userAccName = cleanData(trim($_POST["usrAccountName"]));
					//$userAccType = cleanData(trim($_POST["userAccountType"]));
					
					if ( !empty($userAccName) ) {
						if($_SESSION["username"] === $userAccName){
							print ("Error: You can not remove the account of yourself from the system.");	
						}else{
							$auth_dataArr = array('username' => $_SESSION["username"], 'ip_address' => $_SERVER['REMOTE_ADDR'], 'login_hash' => $_SESSION["loginHash"]);
							$requestArr = array('username' => $userAccName);
							$arr = array('method' => 'removeuser', 'auth_data' => $auth_dataArr,'request' => $requestArr );
							...
							$authNum = $responseArr["response"]["deleted"];
							$timeout = $responseArr["response"]["timeout"];

							if ($method == "removeuser"){
								if ($authNum === 1) {
									print ("<i>$userAccName </i> was successfuly removed from the system.");
								} else if ($authNum === 0){
									if($timeout===1){
										print ("Your session is timeout, you need relogin.");
									}else{
										print ("Error: Could not remove <i>$userAccName </i> from the system.");	
									}
								}
					...
					} else print "Please enter valid user name.";
			  }
			  else {
